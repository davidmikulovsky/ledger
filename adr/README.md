...
- [0007 — Group representation and spin-off model](0007-group-representation-and-spin-off-model.md) — ventures modeled as independent top-level units linked by `parent:` + `portfolio_members:`, not nested `sub_units:`; spin-off = drop the link. Supersedes the `sub_units:` convention. **Field names and delivery refined by ADR-0012.**
- [0008 — Child-safety write boundary between projects](0008-child-safety-write-boundary.md) — a child/sibling session treats parent/sibling folders read-only; destructive ops anywhere need explicit ok + backup. The mirror of ADR-0001 between projects.
- [0009 — Git-lock handling and folder-access policy](0009-git-lock-and-folder-access-policy.md) — read-only by default; David alone grants/revokes access; ask before first git write; lock prevention + cleanup runbook + `_to_delete/` fallback.
- [0010 — Canonical guidelines / shared-resources repo](0010-canonical-guidelines-shared-resources-repo.md) — ALFA+OMEGA (git) is the single source of truth for shared guidelines and reusable resources; versioned by tag + in-doc stamp; Ledger's own guideline copies superseded to pointer stubs.
- [0011 — Scan engine and state model](0011-scan-engine-and-state-model.md) — `ledger scan` writes `ledger-state.json` (health/movement/flags) as the single snapshot spine every other v0.6 skill reads.
- [0012 — Family schema, delivery mechanism, and orphan tiering](0012-family-schema-and-delivery-mechanism.md) — locks `role:`/`parent:`/`relationship:`/`members:` as the canonical fields (retiring `parent_unit:`/`sub_units:`/`portfolio_members:`); adds a `mentions:` tier for non-project entities (domains, placeholders, ideas with no real folder); delivers the schema via `project-standard.md` (guidelines v2.1) instead of only this ADR log.
