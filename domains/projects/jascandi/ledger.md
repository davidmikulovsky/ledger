...
---
**2026-07-21 (v0.3 vocabulary pass)** — `status` corrected from the invented `dormant-capable` to canonical `active`: real work happened this week (location resolution, v2.2 fix, docs catch-up), so activity-wise the answer is unambiguous. `priority` stays `low`, now explicit about why — not incompleteness, just no real business/client stakes yet — matching David's own example when he requested this vocabulary change. `components` block populated as the schema's pilot case.
